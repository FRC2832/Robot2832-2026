...
public class TurretSubsystem_Old extends SubsystemBase {
    ...
    //     leftRotationMotor.setControl(motionMagicRequest.withPosition(Angle.ofRelativeUnits(leftTarget, Units.Degrees)));
    //     rightRotationMotor.setControl(motionMagicRequest.withPosition(Angle.ofRelativeUnits(rightTarget, Units.Degrees)));
    // }
    ...
    //         leftVoltage = MathUtil.clamp(leftVoltage, -12, 0);
    //     //}else if(leftAngle > Constants.LEFT_TURRET_MAX_ANGLE){
    //         leftVoltage = MathUtil.clamp(leftVoltage, 0, 12);
    //     //}else{
